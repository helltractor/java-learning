## Demo-Redis

### Tutorial

* [使用 Testcontainers 测试 Redis](https://springdoc.cn/spring-boot-redis-testcontainers/)
* [testcontainers-java example](https://github.com/testcontainers/testcontainers-java/tree/main/examples/spring-boot)

### Reference

* [redis](https://redis.io/)
    * [redis commands](https://redis.io/commands)
* [spring-data-redis](https://docs.spring.io/spring-data/redis/reference/index.html)
* [testcontainers](https://www.testcontainers.org/modules/redis/)
    * [testcontainers-java](https://github.com/testcontainers/testcontainers-java)